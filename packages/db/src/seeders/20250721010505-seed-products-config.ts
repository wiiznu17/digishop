...
export default {
  async up(q: QueryInterface): Promise<void> {
    ...
    const getItemId = async (productId: number, sku: string) => {
      ...
      if (!rows?.length)
        throw new Error(`Item not found: product_id=${productId}, sku=${sku}`)
    ...
    const getVariationId = async (productId: number, name: string) => {
      ...
      if (!rows?.length)
        throw new Error(
          `Variation not found: product_id=${productId}, name=${name}`
        )
      return rows[0].id as number
    }
    ...
    const getOptionId = async (variationId: number, value: string) => {
      ...
      if (!rows?.length)
        throw new Error(
          `Option not found: variation_id=${variationId}, value=${value}`
        )
      return rows[0].id as number
    }
    ...
    await q.bulkInsert('PRODUCT_CONFIGURATIONS', [
      ...
      {
        uuid: uuidv4(),
        product_item_id: itemXBlk128,
        variation_option_id: optXBlack,
        created_at: now,
        updated_at: now,
        deleted_at: null
      },
      ...
      {
        uuid: uuidv4(),
        product_item_id: itemXSlv256,
        variation_option_id: optXSilver,
        created_at: now,
        updated_at: now,
        deleted_at: null
      },
      ...
      {
        uuid: uuidv4(),
        product_item_id: itemYBlue,
        variation_option_id: optYBlue,
        created_at: now,
        updated_at: now,
        deleted_at: null
      },
      ...
      {
        uuid: uuidv4(),
        product_item_id: itemYBlack,
        variation_option_id: optYBlack,
        created_at: now,
        updated_at: now,
        deleted_at: null
      },
      ...
      {
        uuid: uuidv4(),
        product_item_id: itemL1516512,
        variation_option_id: optL16,
        created_at: now,
        updated_at: now,
        deleted_at: null
      },
      ...
      {
        uuid: uuidv4(),
        product_item_id: itemL15321TB,
        variation_option_id: optL32,
        created_at: now,
        updated_at: now,
        deleted_at: null
      },
      ...
      {
        uuid: uuidv4(),
        product_item_id: itemEBWht,
        variation_option_id: optEBWhite,
        created_at: now,
        updated_at: now,
        deleted_at: null
      },
      ...
      {
        uuid: uuidv4(),
        product_item_id: itemSWS,
        variation_option_id: optBandS,
        created_at: now,
        updated_at: now,
        deleted_at: null
      },
      {
        uuid: uuidv4(),
        product_item_id: itemSWM,
        variation_option_id: optBandM,
        created_at: now,
        updated_at: now,
        deleted_at: null
      },
      {
        uuid: uuidv4(),
        product_item_id: itemSWL,
        variation_option_id: optBandL,
        created_at: now,
        updated_at: now,
        deleted_at: null
      }
      ...
      const [rows]: any = await q.sequelize.query(
        `SELECT id FROM PRODUCT_ITEMS WHERE product_id = :pid AND sku IN (${skus.map((_, i) => ':s' + i).join(',')})`,
        {
          replacements: Object.assign(
            { pid: productId },
            Object.fromEntries(skus.map((s, i) => ['s' + i, s]))
          )
        }
      )
