...
export default {
  async up(queryInterface: QueryInterface) {
    ...
    await queryInterface.addIndex('PAYMENT_GATEWAY_EVENTS', {
      name: 'idx_pge_checkout_id',
      fields: ['checkout_id']
    })
    await queryInterface.addIndex('PAYMENT_GATEWAY_EVENTS', {
      name: 'idx_pge_payment_id',
      fields: ['payment_id']
    })
    await queryInterface.addIndex('PAYMENT_GATEWAY_EVENTS', {
      name: 'idx_pge_refund_order_id',
      fields: ['refund_order_id']
    })
    await queryInterface.addIndex('PAYMENT_GATEWAY_EVENTS', {
      name: 'idx_pge_provider_ref',
      fields: ['provider_ref']
    })
    await queryInterface.addIndex('PAYMENT_GATEWAY_EVENTS', {
      name: 'idx_pge_request_id',
      fields: ['request_id']
    })
    await queryInterface.addIndex('PAYMENT_GATEWAY_EVENTS', {
      name: 'idx_pge_created_at',
      fields: ['created_at']
    })
    await queryInterface.addIndex('PAYMENT_GATEWAY_EVENTS', {
  ...
  async down(queryInterface: QueryInterface) {
    ...
    try {
      await queryInterface.removeIndex(
        'PAYMENT_GATEWAY_EVENTS',
        'idx_pge_provider_ref'
      )
    } catch {}
    try {
      await queryInterface.removeIndex(
        'PAYMENT_GATEWAY_EVENTS',
        'idx_pge_refund_order_id'
      )
    } catch {}
    try {
      await queryInterface.removeIndex(
