...
export default {
  async up(queryInterface: QueryInterface) {
    ...
    await queryInterface.addIndex('REFUND_IMAGES', {
      name: 'idx_refund_images_refund_order_id',
      fields: ['refund_order_id']
    })
    await queryInterface.addIndex('REFUND_IMAGES', {
      name: 'idx_refund_images_refund_order_sort',
      fields: ['refund_order_id', 'sort_order']
    })
  },
  ...
  async down(queryInterface: QueryInterface) {
    try {
      await queryInterface.removeIndex(
        'REFUND_IMAGES',
        'idx_refund_images_refund_order_sort'
      )
    } catch {}
    try {
      await queryInterface.removeIndex(
        'REFUND_IMAGES',
        'idx_refund_images_refund_order_id'
      )
    } catch {}
