...
export default {
  async up(q: QueryInterface): Promise<void> {
    ...
    await q.addIndex('VARIATION_OPTIONS', ['uuid'], {
      unique: true,
      name: 'uq_variation_options_uuid'
    })
    await q.addIndex('VARIATION_OPTIONS', ['variation_id'], {
      name: 'ix_variation_options_variation'
    })
    await q.addIndex('VARIATION_OPTIONS', ['variation_id', 'value'], {
      name: 'ix_variation_options_variation_value'
    })
  },
