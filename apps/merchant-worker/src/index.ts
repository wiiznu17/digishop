...
async function main() {
  ...
  const refundQueue = new Queue(
    ENV.REFUND_QUEUE_NAME || 'refund-auto-approve',
    { connection }
  )
  const cancelQueue = new Queue(ENV.CANCEL_QUEUE_NAME || 'auto-cancel-unpaid', {
    connection
  })
  const completeQueue = new Queue(ENV.COMPLETE_QUEUE_NAME || 'auto-complete', {
    connection
  })
  const refreshTokenQueue = new Queue(ENV.REFRESH_TOKEN || 'refresh-token', {
    connection
  })
  ...
  async function shutdown() {
    try {
      ...
      await Promise.allSettled([
        refundW.worker?.close(),
        refundW.qe?.close(),
        // cw.worker?.close(),  cancelW.qe?.close(),
        completeW.worker?.close(),
        completeW.qe?.close(),
        refreshW.worker?.close(),
        refreshW.qe?.close()
      ])
      // ปิด queues + redis
      await Promise.allSettled([
