...
export function startRefundWorker(connection: IORedis) {
  ...
  const worker = new Worker<RefundJob>(
    ...
    async (job) => {
      ...
      try {
        ...
        logger.info({ jobId: job.id, orderId, status: res.data?.data?.status }, "refund auto-approved");
        return res.data;
      } catch (err: any) {
  ...
  qe.on("completed", ({ jobId }) => logger.info({ jobId }, "refund job completed"));
  qe.on("failed", async ({ jobId, failedReason }) => {
    ...
    if (attemptsMade >= maxAttempts) {
      await dlq.add("dead", job.data, { jobId: `dead-${job.id}`, removeOnComplete: true });
