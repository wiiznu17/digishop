...
export function startAutoCancelWorker(connection: IORedis) {
  ...
  const worker = new Worker<CancelJob>(
    ...
    async (job) => {
      ...
      const pay = await Payment.findOne({
        where: { checkoutId: Number(order.get('checkoutId')) },
        attributes: ['status']
      })
      ...
      const res =
        ...
        // );
        await http.patch(`/api/customer/order/customer/cancel/${orderId}`)

      logger.info(
        { jobId: job.id, orderId, newStatus: res.data?.data?.status },
        'auto-cancel executed'
      )
      return res.data
    },
  ...
  qe.on('completed', ({ jobId, returnvalue }) =>
    logger.info({ jobId, returnvalue }, 'auto-cancel completed')
  )
  qe.on('failed', ({ jobId, failedReason }) =>
    logger.error({ jobId, failedReason }, 'auto-cancel failed')
  )
