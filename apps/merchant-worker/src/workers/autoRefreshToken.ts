...
export function startAutoRefreshToken(connection: IORedis) {
  ...
  qe.on("completed", ({ jobId, returnvalue }) => logger.info({ jobId, returnvalue }, "refresh-token completed"));
  qe.on("failed", ({ jobId, failedReason }) => logger.error({ jobId, failedReason }, "refresh-token failed"));
