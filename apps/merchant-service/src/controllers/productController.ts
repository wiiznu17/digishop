...
export const suggestProducts = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    const result = await productService.suggestProducts(
      storeId,
      toQueryString(req.query.q)
    )
...
export const bulkUpdateProductStatus = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    const storeId = requireStoreId(req, 'No store found')
    const { productUuids, status } = req.body as {
      productUuids: string[]
      status: string
    }
...
export const updateProductItem = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    const storeId = requireStoreId(req, 'No store found')
    const { productUuid, itemUuid } = req.params as {
      productUuid: string
      itemUuid: string
    }
...
// ===== public handlers =====
export const syncCreateDesiredProduct = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    const storeId = requireStoreId(req, 'No store found')
...
export const syncUpdateDesiredProduct = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    const storeId = requireStoreId(req, 'No store found')
