...
const pickUploadBlobByKey = (
  ...
  return files.find((file) =>
    String(file.originalname || '').startsWith(`${uploadKey}__`)
  )
}
...
const makeComboKeyFromItem = (
  ...
  const pairs = (item.configurations ?? [])
    ...
    .filter(
      (pair): pair is { variationId: number; optionUuid: string } =>
        !!pair.variationId && !!pair.optionUuid
    )

  const sorted = pairs.sort(
    (a, b) =>
      (varIndexById.get(a.variationId) ?? 0) -
      (varIndexById.get(b.variationId) ?? 0)
  )
...
export class ProductService {
  ...
  private async ensureMainImage(productId: number, transaction: Transaction) {
    const hasMain = await productRepository.countMainProductImages(
      productId,
      transaction
    )
    if (hasMain) return

    const firstImage = await productRepository.findFirstProductImage(
      productId,
      transaction
    )
    if (firstImage) {
      await productRepository.setProductImageMainById(
        firstImage.id,
        transaction
      )
    }
  ...
  private async buildFootprintFromDB(
    productId: number,
    transaction: Transaction
  ): Promise<ReviewFootprint> {
    ...
    variations.forEach((variation, index) =>
      varIndexById.set(variation.id, index)
    )
  ...
  private buildFootprintFromDesired(
    desired: DS_Payload,
    ...
    const desiredMainUuid =
      desired.images.product.find((img) => img.isMain && img.uuid)?.uuid ?? null
    ...
    return {
      ...
      description:
        (desired.product.description ?? null)?.toString().trim() || null,
  ...
  async suggestProducts(storeId: number, q: string | undefined) {
    ...
    const productIds = Array.from(
      new Set<number>([
        ...
        ...skuRows
          .map(
            (skuRow) =>
              (skuRow as ProductItem & { product?: Product }).product?.id
          )
    ...
    if (productIds.length > 0) {
      const mainImages =
        await productRepository.findMainImagesByProductIds(productIds)
  ...
  async getProductList(
    ...
    const pageSizeRaw = Math.max(
      parseInt(String(rawQuery.pageSize ?? '20'), 10) || 20,
      1
    )
  ...
  async getProductDetail(storeId: number, productUuid: string) {
    const product = await productRepository.findProductDetailByUuidAndStore(
      productUuid,
      storeId
    )
    if (!product) {
  ...
  async applyDesiredState(input: ApplyDesiredStateInput) {
    ...
    try {
      ...
      if (mode === 'create') {
        const categoryId = await productRepository.mapCategoryUuid(
          desired.product.categoryUuid ?? null,
          transaction
        )
        if (!categoryId) {
          throw new ProductServiceError(
            400,
            'categoryUuid is required or invalid'
          )
        }
        ...
        product = await productRepository.findProductForUpdate(
          productUuid,
          storeId,
          transaction
        )
        ...
        if (desired.ifMatchUpdatedAt) {
          ...
          if (
            currentUpdatedAt !==
            new Date(desired.ifMatchUpdatedAt).toISOString()
          ) {
            throw new ProductServiceError(
              409,
        ...
        const beforeFootprint = await this.buildFootprintFromDB(
          product.id,
          transaction
        )
      ...
      const currentImages =
        await productRepository.findProductImagesByProductId(
          productId,
          transaction,
          true
        )
      ...
      const createdImageByKey = new Map<
        string,
        Awaited<ReturnType<typeof productRepository.createProductImage>>
      >()
        ...
        const created = await productRepository.createProductImage(
          {
            ...
            blobName,
            fileName:
              file.originalname.split('__').slice(1).join('__') ||
              file.originalname,
      ...
      if (desiredMainUuid) {
        await productRepository.clearMainProductImagesExcept(
          productId,
          desiredMainUuid,
          transaction
        )
        await productRepository.setMainProductImage(
          productId,
          desiredMainUuid,
          transaction
        )
      ...
      const currentVariations =
        await productRepository.findVariationsByProductId(
          productId,
          transaction,
          true
        )
      ...
      const desiredVariationUuidSet = new Set(
        desired.variations
          .map((variation) => variation.uuid)
          .filter((uuid): uuid is string => !!uuid)
      )

      for (const currentVariation of currentVariations) {
        if (
          currentVariation.uuid &&
          !desiredVariationUuidSet.has(currentVariation.uuid)
        ) {
          await productRepository.deleteVariationById(
            currentVariation.id,
            transaction
          )
        }
      ...
      for (const desiredVariation of desired.variations) {
        ...
        if (desiredVariation.uuid) {
          variationRecord =
            await productRepository.findVariationByUuidForProduct(
          ...
          variationRecord = await productRepository.createVariation(
            productId,
            desiredVariation.name,
            transaction
          )
        ...
        const currentOptions =
          await productRepository.findVariationOptionsByVariationId(
            variationRecord.id,
            transaction,
            true
          )
        ...
        for (const currentOption of currentOptions) {
          if (
            currentOption.uuid &&
            !desiredOptionUuidSet.has(currentOption.uuid)
          ) {
            await productRepository.deleteVariationOptionById(
              currentOption.id,
              transaction
            )
          }
        ...
        for (const desiredOption of desiredVariation.options) {
          if (desiredOption.uuid) {
            const existingOption =
              await productRepository.findVariationOptionByUuidForVariation(
                desiredOption.uuid,
                variationRecord.id,
                transaction
              )
            ...
            if (
              (existingOption.sortOrder ?? 0) !== (desiredOption.sortOrder ?? 0)
            ) {
              updatePayload.sortOrder = desiredOption.sortOrder ?? 0
            }

            if (Object.keys(updatePayload).length > 0) {
              await productRepository.updateVariationOptionById(
                existingOption.id,
                updatePayload,
                transaction
              )
            ...
            if (desiredOption.clientId) {
              optionUuidByClientId.set(
                desiredOption.clientId,
                existingOption.uuid
              )
            }
      ...
      const finalVariations =
        await productRepository.findFinalVariationsForCombo(
          productId,
          transaction
        )
      ...
      const varIndexById = new Map<number, number>()
      {
        const variationByName = new Map(
          finalVariations.map((variation) => [variation.name, variation])
        )

        desired.variations.forEach((desiredVariation, index) => {
          const matched =
            (desiredVariation.uuid &&
              finalVariations.find(
                (variation) => variation.uuid === desiredVariation.uuid
              )) ||
      ...
      const optionUuidMatrix = finalVariations
        .sort(
          (a, b) =>
            (varIndexById.get(a.id) ?? 0) - (varIndexById.get(b.id) ?? 0)
        )
        .map((variation) =>
          (variation.options ?? [])
      ...
      const allowedComboKeys = new Set<string>(
        optionUuidMatrix.length === 0 ||
        optionUuidMatrix.some((row) => row.length === 0)
          ? []
          : cartesian(optionUuidMatrix).map((row) => row.join('|'))
      )

      const currentItems = await productRepository.findProductItemsByProductId(
        productId,
        transaction,
        true
      )
      ...
      for (const variation of desired.variations) {
        if ((variation.options?.length ?? 0) < 1) {
          throw new ProductServiceError(
            400,
            `Variation "${variation.name}" must have at least 1 option`
          )
        }
      }
      ...
      for (const desiredItem of desired.items) {
        ...
        if (!desiredItem.uuid && !sku) {
          throw new ProductServiceError(
            400,
            `SKU is required (clientKey=${desiredItem.clientKey ?? '-'})`
          )
        }
        ...
        if (desiredItem.uuid && sku.length === 0) {
          throw new ProductServiceError(
            400,
            `SKU is required for item uuid=${desiredItem.uuid}`
          )
        }
        ...
        if (desiredItem.image) {
          const existing =
            await productRepository.findProductItemImageByProductItemId(
              itemRecord.id,
              transaction
            )
          ...
          if (desiredItem.image.remove === true) {
            ...
            const file = pickUploadBlobByKey(
              filesI,
              desiredItem.image.uploadKey
            )
            if (file) {
              if (existing) {
                await azureBlobService.deleteImage(existing.blobName)
        ...
        if (optionUuids.length) {
          const options =
            await productRepository.findVariationOptionsByUuidsForProduct(
              optionUuids,
              productId,
              transaction
            )
          ...
          const existingConfigs =
            await productRepository.findProductConfigurationsByItemId(
              itemRecord.id,
              transaction
            )
          const existingIds = new Set(
            existingConfigs.map((config) => config.variationOptionId)
          )
          ...
          for (const addId of toAdd) {
            if (allowedIds.has(addId)) {
              await productRepository.createProductConfiguration(
                itemRecord.id,
                addId,
                transaction
              )
          ...
          if (toDelete.length) {
            await productRepository.deleteProductConfigurationsByOptionIds(
              itemRecord.id,
              toDelete,
              transaction
            )
      ...
      const detail = await productRepository.findProductDetailByUuid(
        product.uuid
      )
      return {
  ...
  async deleteProduct(storeId: number, productUuid: string) {
    ...
    try {
      const product = await productRepository.findProductForDelete(
        productUuid,
        storeId,
        transaction
      )
  ...
  async bulkUpdateProductStatus(
    ...
    const [updated] = await productRepository.bulkUpdateProductStatus(
      productUuids,
      coercedStatus,
      storeId
    )
  ...
  async bulkDeleteProducts(storeId: number, productUuids: string[]) {
    ...
    try {
      ...
      const products = await productRepository.findProductsForBulkDelete(
        productUuids,
        storeId,
        transaction
      )
      const blobs = products.flatMap((product) =>
        (product.images ?? []).map((image) => image.blobName)
      )
      ...
      await productRepository.destroyProductsForBulkDelete(
        productUuids,
        storeId,
        transaction
      )
  ...
  async duplicateProduct(storeId: number, productUuid: string) {
    ...
    try {
      const source =
        await productRepository.findProductWithRelationsByUuidForStore(
          productUuid,
          storeId,
          transaction
        )
      ...
      if (source.variations?.length) {
        for (const variation of source.variations) {
          const newVariation = await productRepository.createVariation(
            destination.id,
            variation.name,
            transaction
          )
