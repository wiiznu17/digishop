...
export class BankServiceError extends AppError {
  ...
  constructor(statusCode: number, body: Record<string, unknown>) {
    super(
      String(body.error ?? body.message ?? 'Bank service error'),
      statusCode,
      true,
      body
    )
...
export class BankService {
  ...
  async addBankAccountToStore(input: AddBankAccountInput) {
    ...
    try {
      ...
      const { accountHolderName, accountNumber, bankName, isDefault } =
        input.payload
  ...
  async setDefaultBankAccount(input: SetDefaultBankAccountInput) {
    ...
    try {
      ...
      const accountToSetDefault =
        await bankRepository.findBankAccountByIdAndStore(
          input.accountId,
          storeId,
          transaction
        )
      ...
      if (!accountToSetDefault) {
        throw new BankServiceError(404, {
          error: 'Bank account not found in your store.'
        })
      }

      await setDefaultAccountForStore(
        storeId,
        accountToSetDefault.id,
        transaction
      )
      await transaction.commit()

      const updatedAccount = await bankRepository.findBankAccountByPk(
        accountToSetDefault.id
      )
      return {
  ...
  async deleteBankAccount(input: DeleteBankAccountInput) {
    ...
    const accountToDelete = await bankRepository.findBankAccountByIdAndStore(
      input.bankAccountId,
      storeId
    )
    if (!accountToDelete) {
    ...
    if (accountToDelete.isDefault) {
      throw new BankServiceError(400, {
        error:
          'Cannot delete the default bank account. Please set another account as default first.'
      })
    }
