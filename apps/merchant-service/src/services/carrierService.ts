...
export class CarrierServiceError extends AppError {
  ...
  constructor(statusCode: number, body: Record<string, unknown>) {
    super(
      String(body.error ?? body.message ?? 'Carrier service error'),
      statusCode,
      true,
      body
    )
...
const SHIP_ALLOW_NEXT: Partial<
  Record<ShippingStatus, ReadonlyArray<ShippingStatus>>
> = {
  ...
  [ShippingStatus.OUT_SORTING_CENTER]: [
    ShippingStatus.ARRIVED_DESTINATION_STATION
  ],
  [ShippingStatus.ARRIVED_DESTINATION_STATION]: [
...
const ORDER_ALLOW_NEXT: Partial<
...
const canShipTransition = (
  from: ShippingStatus | null,
  to: ShippingStatus
): boolean => {
...
export class CarrierService {
  private async moveOrderStatus(
    ...
    await carrierRepository.updateOrderStatus(
      orderRecord,
      toStatus,
      transaction
    )
  ...
  async processWebhook(
    input: CarrierWebhookInput
  ): Promise<CarrierWebhookSuccessResponse> {
    ...
    await carrierRepository.withTransaction(async (transaction) => {
      ...
      if (nextShippingStatus === ShippingStatus.TRANSIT_ISSUE) {
        const outForDeliveryCount =
          await carrierRepository.countShipmentEventsByToStatus(
            shipping.id,
            ShippingStatus.OUT_FOR_DELIVERY,
            transaction
          )
            ...
            await carrierRepository.createShipmentEvent(
              {
                ...
                description:
                  'Auto transition after 3 OUT_FOR_DELIVERY attempts',
                location: null,
      ...
      if (order && nextOrderStatus) {
        ...
        if (nextOrderStatus === OrderStatus.TRANSIT_LACK) {
          const shippedCount =
            await carrierRepository.countOrderStatusHistoryByToStatus(
              order.id,
              OrderStatus.SHIPPED,
              transaction
            )
