...
class OrderServiceError extends AppError {
  ...
  constructor(statusCode: number, body: Record<string, unknown>) {
    super(
      String(body.error ?? body.message ?? 'Order service error'),
      statusCode,
      true,
      body
    )
...
const toInt = (value: unknown, def: number) =>
...
function read<T = unknown>(obj: unknown, ...keys: string[]): T | undefined {
  ...
  for (const key of keys) {
    const viaGet =
      typeof source.get === 'function' ? source.get(key) : undefined
...
const SORT_WHITELIST = new Set<OrderSortField>([
  'id',
  'createdAt',
  'updatedAt',
  'grandTotalMinor'
...
const pathRefund = (ref: string) =>
  `/transaction/${encodeURIComponent(ref)}/refund`
...
function serializeOrder(order: Order): SerializedOrder {
  ...
  const histories = asArray<Record<string, unknown>>(
    read(order, 'statusHistory')
  )
  const refundOrders = asArray<Record<string, unknown>>(
    read(order, 'refundOrders')
  )

  const grandMinor =
    read<number>(order, 'grand_total_minor', 'grandTotalMinor') ?? 0
  const shippingSnap = read<number>(
    shipping,
  ...
  const addrSnap = read<Record<string, unknown>>(
    shipping,
    'address_snapshot',
    'addressSnapshot'
  )
  ...
  const orderItems = items.map((item) => {
    ...
    return {
      ...
      price: minorTo(
        read<number>(item, 'unit_price_minor', 'unitPriceMinor'),
        2
      ),
      discount: minorTo(
        read<number>(item, 'discount_minor', 'discountMinor'),
        2
      ),
  ...
  const shippingEvents = asArray<Record<string, unknown>>(
  ...
  const returnShipments = asArray<Record<string, unknown>>(
    read(order, 'returnShipments')
  ).map((shipment) => ({
    ...
    trackingNumber: (read<string>(shipment, 'trackingNumber') ?? null) as
      | string
      | null,
    shippedAt: (read<Date>(shipment, 'shippedAt') ?? null) as Date | null,
    deliveredBackAt: (read<Date>(shipment, 'deliveredBackAt') ??
      null) as Date | null,
    fromAddressSnapshot: read(shipment, 'fromAddressSnapshot') ?? null,
    toAddressSnapshot: read(shipment, 'toAddressSnapshot') ?? null,
    events: asArray<Record<string, unknown>>(read(shipment, 'events')).map(
      (event) => ({
        id: Number(read(event, 'id') ?? 0),
        fromStatus: (read<string>(event, 'fromStatus') ?? null) as
          | string
          | null,
        toStatus: (read<string>(event, 'toStatus') ?? '') as string,
        description: (read<string>(event, 'description') ?? null) as
          | string
          | null,
        location: (read<string>(event, 'location') ?? null) as string | null,
        occurredAt: read<Date>(event, 'occurredAt') as Date,
  ...
  return {
    ...
    status: order.status,
    statusHistory: histories.map((history) =>
      String(read(history, 'toStatus') ?? '')
    ),
    currency,
    subtotal: minorTo(
      read<number>(order, 'subtotal_minor', 'subtotalMinor'),
      2
    ),
    shippingCost: minorTo(
      shippingSnap ??
    ...
    discount: minorTo(
      read<number>(order, 'discount_total_minor', 'discountTotalMinor'),
      2
    ),
    grandTotal: minorTo(grandMinor, 2),
    paymentMethod:
      read<string>(payment, 'payment_method', 'paymentMethod') ?? '',
    payment: Object.keys(payment).length
      ? {
          provider: read<string>(payment, 'provider', 'provider') ?? undefined,
          providerRef:
            read<string>(payment, 'provider_ref', 'providerRef') ?? undefined,
          channel: read<string>(payment, 'channel', 'channel') ?? undefined,
          pgwStatus:
            read<string>(payment, 'pgw_status', 'pgwStatus') ?? undefined,
    ...
    returnedToSenderAt: read<Date>(
      shipping,
      'returned_to_sender_at',
      'returnedToSenderAt'
    ),
    shippingStatus: read<ShippingStatus>(
      shipping,
      'shipping_status',
      'shippingStatus'
    ),
...
export class OrderService {
  ...
  async getOrdersSummary(
    ...
    const totalRevenueMinor = revenueOrders.reduce(
      (sum, order) => sum + (order.grandTotalMinor ?? 0),
      0
    )
  ...
  async getOrderDetail(storeId: number | undefined, orderId: string) {
    ...
    if (storeId) {
      const owned = await orderRepository.findOrderByIdAndStore(
        orderId,
        storeId
      )
      if (owned === 0) {
        throw new OrderServiceError(403, {
  ...
  async getOrderList(storeId: number | undefined, query: OrderListQuery) {
    ...
    const orderField: OrderSortField = SORT_WHITELIST.has(
  ...
  async updateOrderStatus(input: UpdateOrderInput) {
    ...
    const changedById = Number.isFinite(Number(changedByIdRaw))
    ...
    if (!isService && !storeId) {
      throw new OrderServiceError(403, {
        error: 'Forbidden: Store context required'
      })
    }
    ...
    try {
      const order = await orderRepository.findOrderByPkWithBaseIncludes(
        orderId,
        transaction
      )
      if (!order) {
      ...
      if (ship && newShipStatus) {
        const now = new Date()
        const lastEvent = await orderRepository.findLatestShipmentEvent(
          ship.id,
          transaction
        )
        const fromStatus =
          (lastEvent?.get('toStatus') as ShippingStatus | undefined) ??
          ship.shippingStatus ??
          null
      ...
      if (nextStatus === OrderStatus.RETURN_VERIFIED) {
        if (current !== OrderStatus.RECEIVE_RETURN) {
          throw new OrderServiceError(409, {
            error: 'order_not_in_receive_return'
          })
        }

        const now = new Date()
        const ret = await orderRepository.findReturnShipmentByOrderId(
          order.id,
          transaction
        )
        ...
        const useRtsEvent =
          !useReturnEvent &&
          ship &&
          ship.shippingStatus === ShippingStatus.RETURNED_TO_SENDER
      ...
      let refund = await orderRepository.findRefundOrderByOrderId(
        order.id,
        transaction
      )
      if (
        ...
        if (!refund && nextStatus === OrderStatus.MERCHANT_CANCELED) {
          refund = await orderRepository.createRefundOrder(
            {
              ...
              approvedAt:
                nextStatus === OrderStatus.MERCHANT_CANCELED
                  ? new Date()
                  : null,
          ...
          if (
            nextStatus === OrderStatus.REFUND_APPROVED &&
            ...
            nextStatus === OrderStatus.REFUND_SUCCESS &&
            ...
            nextStatus === OrderStatus.REFUND_FAIL &&
            ...
            nextStatus === OrderStatus.AWAITING_RETURN &&
            refund.status === RefundStatus.REQUESTED
          ) {
            if (!ship || ship.shippingStatus !== ShippingStatus.DELIVERED) {
              throw new OrderServiceError(409, {
                error: 'order_not_delivered_cannot_request_return'
              })
            }

            let ret = await orderRepository.findReturnShipmentByOrderId(
              order.id,
              transaction
            )
            if (ret) {
              throw new OrderServiceError(409, {
                error: 'return_shipment_already_exists'
              })
            }
            ...
            const fromAddressSnapshot =
              read(ship, 'address_snapshot', 'addressSnapshot') ?? null
            ...
            await orderRepository.createReturnShipmentEvent(
              {
                ...
                occurredAt: new Date(),
                description:
                  'Return created (REFUND_REQUEST -> AWAITING_RETURN)'
              },
              transaction
            )
          } else if (
            nextStatus === OrderStatus.REFUND_REJECTED &&
      ...
      if (
        ...
        const refundRow =
          refund ??
          (await orderRepository.findRefundOrderByOrderId(
            order.id,
            transaction
          ))
        ...
        postCommit = async () => {
          ...
          try {
            const detail = await pgwGetDetail(
              reference,
            ...
            if (action === 'NONE') {
              ...
              await orderRepository.createPaymentGatewayEvent({
                checkoutId: (read<number>(checkout, 'id') ?? null) as
                  | number
                  | null,
                paymentId,
            ...
            await orderRepository.createPaymentGatewayEvent({
              ...
              resJson: pgwError.response?.data ?? {
                message: pgwError.message ?? 'unknown'
              }
            })
          }
        }
      ...
      const fresh =
        await orderRepository.findFreshOrderWithBaseIncludes(orderId)
