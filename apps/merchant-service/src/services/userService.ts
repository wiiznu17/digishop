...
export class UserServiceError extends AppError {
  ...
  constructor(statusCode: number, body: Record<string, unknown>) {
    super(
      String(body.error ?? body.message ?? 'User service error'),
      statusCode,
      true,
      body
    )
...
export class UserService {
  ...
  async getMerchantProfile(input: GetMerchantProfileInput) {
    ...
    const merchantProfile =
      await userRepository.findMerchantProfileByUserId(userId)
    if (!merchantProfile) {
    ...
    if (storeId) {
      ...
      const memberSince =
        (profileJson.store?.createdAt as string | Date | null | undefined) ??
        null
  ...
  async updateMerchantProfile(input: UpdateMerchantProfileInput) {
    ...
    try {
      ...
      const profileData = JSON.parse(
        input.profileDataString
      ) as UpdateMerchantProfilePayload
      const files = input.files ?? []

      const user = await userRepository.findUserByPk(
      ...
      const storeRecord = await userRepository.findStoreByPk(
        storeId,
        transaction
      )
      if (!storeRecord) {
      ...
      const existingImages = await userRepository.findProfileImagesByStoreId(
        storeRecord.id,
        transaction
      )
      ...
      const deleteExistingImages = async () => {
        ...
        await userRepository.destroyProfileImagesByStoreId(
          storeRecord.id,
          transaction
        )
      }
  ...
  private async syncMerchantAddresses(
    ...
    const existingRows = await userRepository.findMerchantAddressesByStoreId(
      storeId,
      transaction
    )
    ...
    if (idsToDelete.length > 0) {
      await userRepository.destroyMerchantAddressesByIds(
        storeId,
        idsToDelete,
        transaction
      )
    ...
    for (const address of addresses) {
      ...
      if (Number.isFinite(addressId)) {
        await userRepository.updateMerchantAddressById(
          storeId,
          addressId,
          patch,
          transaction
    ...
    if (!defaultAssigned) {
      const firstAddress =
        await userRepository.findFirstMerchantAddressByStoreId(
          storeId,
          transaction
        )
      if (firstAddress) {
        await userRepository.setMerchantAddressDefaultById(
          storeId,
          firstAddress.id,
          transaction
        )
  ...
  async updateMerchantAddress(input: UpdateMerchantAddressInput) {
    ...
    try {
      ...
      const address = await userRepository.findMerchantAddressByIdAndStore(
        addressId,
        store.id,
        transaction
      )
      ...
      if (body.isDefault === true) {
        await userRepository.clearDefaultMerchantAddressesExcept(
          store.id,
          addressId,
          transaction
        )
