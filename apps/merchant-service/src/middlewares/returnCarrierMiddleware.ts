...
export function rcmwVerifySignature(
  req: Request,
  res: Response,
  next: NextFunction
) {
  ...
  const canonical =
    req.returnCarrierCtx?.payloadRaw || JSON.stringify(req.body ?? {})
  const mac = crypto
...
export function rcmwExtractCoreFields(
  req: Request,
  res: Response,
  next: NextFunction
) {
...
export async function rcmwLoadReturnShipment(
  req: Request,
  res: Response,
  next: NextFunction
) {
  ...
  if (!rs)
    return res
      .status(202)
      .json({ ok: true, skipped: 'return_shipment_not_found' })
...
export async function rcmwDeduplicateEvent(
  req: Request,
  _res: Response,
  next: NextFunction
) {
  ...
  req.returnCarrierCtx!.lastEventStatus =
    (lastEvent?.get('toStatus') as ReturnShipmentStatus | undefined) ??
    (rs.status as ReturnShipmentStatus) ??
    null

  if (
    lastEvent &&
    lastEvent.get('toStatus') === req.returnCarrierCtx!.nextReturnStatus
  ) {
    const diffMs = Math.abs(
      new Date(lastEvent.get('occurredAt') as any).getTime() -
        req.returnCarrierCtx!.eventTime.getTime()
    )
...
export async function rcmwComputeOrderTransition(
  req: Request,
  _res: Response,
  next: NextFunction
) {
