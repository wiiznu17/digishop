...
export class ProductRepository {
  ...
  async mapCategoryUuid(uuid?: string | null, transaction?: Transaction): Promise<number | null> {
    if (!uuid) return null;
  ...
  async createProduct(payload: ProductCreationAttributes, transaction: Transaction) {
    return Product.create(payload, { transaction });
  }

  async findProductForUpdate(productUuid: string, storeId: number, transaction: Transaction) {
  ...
  async updateProduct(product: Product, payload: Partial<ProductAttributes>, transaction: Transaction) {
    return product.update(payload, { transaction });
  }

  async findProductByUuidForStore(productUuid: string, storeId: number, transaction?: Transaction) {
    return Product.findOne({ where: { uuid: productUuid, storeId }, transaction });
  }

  async findItemByUuidForProduct(itemUuid: string, productId: number, transaction?: Transaction) {
    return ProductItem.findOne({ where: { uuid: itemUuid, productId }, transaction });
  }
  ...
  async clearMainProductImagesExcept(productId: number, imageUuid: string, transaction: Transaction) {
  ...
  async setMainProductImage(productId: number, imageUuid: string, transaction: Transaction) {
    return ProductImage.update(
      { isMain: true },
  ...
  async countMainProductImages(productId: number, transaction: Transaction) {
    return ProductImage.count({ where: { productId, isMain: true }, transaction });
  }
  ...
  async setProductImageMainById(imageId: number, transaction: Transaction) {
    return ProductImage.update({ isMain: true }, { where: { id: imageId }, transaction });
  }
  ...
  async findVariationByUuidForProduct(variationUuid: string, productId: number, transaction: Transaction) {
    return Variation.findOne({ where: { uuid: variationUuid, productId }, transaction });
  }

  async createVariation(productId: number, name: string, transaction: Transaction) {
    return Variation.create({ productId, name }, { transaction });
  }

  async updateVariationNameById(variationId: number, name: string, transaction: Transaction) {
    return Variation.update({ name }, { where: { id: variationId }, transaction });
  }
  ...
  async findVariationOptionByUuidForVariation(optionUuid: string, variationId: number, transaction: Transaction) {
    return VariationOption.findOne({ where: { uuid: optionUuid, variationId }, transaction });
  }
  ...
  async updateVariationOptionById(
    ...
    return VariationOption.update(payload, { where: { id: optionId }, transaction });
  }
  ...
  async findFinalVariationsForCombo(productId: number, transaction: Transaction) {
    return Variation.findAll({
      where: { productId },
  ...
  async deleteProductConfigurationsByItemId(productItemId: number, transaction: Transaction) {
    return ProductConfiguration.destroy({ where: { productItemId }, transaction });
  }
  ...
  async findProductItemByUuidForProduct(itemUuid: string, productId: number, transaction: Transaction) {
    return ProductItem.findOne({ where: { uuid: itemUuid, productId }, transaction });
  }
  ...
  async updateProductItemById(
    ...
    return ProductItem.update(payload, { where: { id: productItemId }, transaction });
  }

  async findProductItemImageByProductItemId(productItemId: number, transaction: Transaction) {
    return ProductItemImage.findOne({ where: { productItemId }, transaction });
  }

  async deleteProductItemImageById(productItemImageId: number, transaction: Transaction) {
    return ProductItemImage.destroy({ where: { id: productItemImageId }, transaction });
  }

  async createProductItemImage(
    payload: { productItemId: number; url: string; blobName: string; fileName: string },
    transaction: Transaction,
  ) {
  ...
  async findProductConfigurationsByItemId(productItemId: number, transaction: Transaction) {
    return ProductConfiguration.findAll({ where: { productItemId }, transaction });
  }

  async createProductConfiguration(productItemId: number, variationOptionId: number, transaction: Transaction) {
    return ProductConfiguration.create({ productItemId, variationOptionId }, { transaction });
  }
          ...
          include: [
            {
              ...
              as: "options",
              attributes: ["uuid", "value", "sortOrder", "createdAt", "updatedAt"],
            },
          ],
        },
        {
          ...
          include: [
            {
              model: VariationOption,
              as: "options",
              attributes: ["id", "uuid", "sortOrder", "value", "createdAt", "updatedAt"],
            },
          ],
        },
        {
  ...
  async findProductsList(storeId: number, params: ProductListQuery) {
    ...
    const group = [col("Product.id"), col("category.id"), col("category.uuid"), col("category.name")];
  ...
  async findProductForDelete(productUuid: string, storeId: number, transaction: Transaction) {
    return Product.findOne({
      where: { uuid: productUuid, storeId },
      include: [
        { model: ProductImage, as: "images", attributes: ["blobName"] },
        {
          ...
          as: "items",
          attributes: ["id"],
          include: [{ model: ProductItemImage, as: "productItemImage", attributes: ["blobName"], required: false }],
        },
      ],
      transaction,
      lock: transaction.LOCK.UPDATE,
  ...
  async bulkUpdateProductStatus(productUuids: string[], status: ProductStatus, storeId: number) {
  ...
  async findProductsForBulkDelete(productUuids: string[], storeId: number, transaction: Transaction) {
    return Product.findAll({
      where: { uuid: { [Op.in]: productUuids }, storeId },
  ...
  async destroyProductsForBulkDelete(productUuids: string[], storeId: number, transaction: Transaction) {
    return Product.destroy({ where: { uuid: { [Op.in]: productUuids }, storeId }, transaction });
  }
