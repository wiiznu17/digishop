...
export class OrderRepository {
  ...
  async countOrdersByStoreAndDate(
    filter: StoreDateFilter,
  ...
  async findOrderGrandTotalsByStoreAndDate(
    filter: StoreDateFilter,
    transaction?: Transaction
  ) {
    const where = this.buildWhereFromStoreDate(filter)
  ...
  async findOrdersGrandTotalMinor(
    where: WhereOptions,
    transaction?: Transaction
  ) {
    return Order.findAll({
  ...
  async findOrderByPkWithBaseIncludes(
    orderId: number | string,
    transaction?: Transaction
  ) {
    return Order.findByPk(orderId, {
      include: buildOrderBaseIncludes(),
      transaction
    })
  }

  async findOrderByIdAndStore(
    orderId: number | string,
    storeId: number,
    transaction?: Transaction
  ) {
  ...
  async findAndCountOrdersForList(input: FindAndCountOrdersInput) {
    ...
    const shippingBase = baseIncludes.find(
      (include) => include.as === 'shippingInfo'
    ) as IncludeOptions
    ...
    const includes: IncludeOptions[] = [
      ...
      shippingInclude,
      ...baseIncludes.filter(
        (include) => !['checkout', 'shippingInfo'].includes(String(include.as))
      )
    ]
  ...
  async createRefundOrder(
    payload: CreationAttributes<RefundOrder>,
    transaction?: Transaction
  ) {
  ...
  async findReturnShipmentByOrderId(
    orderId: number,
    transaction?: Transaction
  ) {
  ...
  async createReturnShipment(
    payload: CreationAttributes<ReturnShipment>,
    transaction?: Transaction
  ) {
  ...
  async createPaymentGatewayEvent(
    payload: CreationAttributes<PaymentGatewayEvent>
  ) {
