...
export class UserService {
  async createAddress(payload: {
    ...
    const hasDefaultAdd = await userRepository.findDefaultAddress(
      payload.userId
    )
  ...
  async updateAddress(
    ...
    if (payload.isDefault === true) {
      const findIsDefault = await userRepository.findDefaultAddressOne(
        Number(userId)
      )
      if (findIsDefault) {
        await userRepository.updateAddressById(findIsDefault.id, {
          isDefault: false
        })
      }
    }
  ...
  async updateUserName(
    id: number,
    payload: { firstName: string; lastName: string; middleName?: string }
  ) {
