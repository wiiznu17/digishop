...
export class PaymentService {
  async handleNotify(body: {
    ...
    const findPaymentId = await paymentRepository.findPaymentByProviderRef(
      body.reference
    )
    ...
    if (body.status === 'APPROVED' && checkOutId && paymentId) {
      await paymentRepository.updateOrdersByCheckout(checkOutId, {
  ...
  async getCallbackRedirectUrl(
    ...
    const findPaymentId =
      await paymentRepository.findPaymentByProviderRef(reference)
