...
export class ProductService {
  async searchProduct(query: string, page?: number) {
    const productCount = await productRepository.countProductsByName(query as string);

    let searchProduct;
    if (page) {
      searchProduct = await productRepository.searchProductsWithPage(query as string, Number(page));
    } else {
      searchProduct = await productRepository.searchProductNames(query as string);
    }

    const searchStore = await productRepository.searchStoresByName(query as string);
