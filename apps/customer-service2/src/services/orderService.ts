...
export class OrderService {
  ...
  async createOrderId(customerId: string, orderData: any[]) {
    ...
    if (Object.keys(groupStoreId).length > 1)
      throw new BadRequestError('Order more than one store')

    const orderCod = 'DGS' + Date.now() + customerId
    const checkoutId = await orderRepository.createCheckout(
      customerId,
      orderCod
    )
    ...
    for (const [key, values] of Object.entries(groupStoreId) as [
      string,
      any[]
    ][]) {
  ...
  async createOrder(payload: {
    ...
    if (!user || !checkoutId)
      throw new BadRequestError('User or CheckOut not found')
    ...
    for (const ord of orderId) {
      await orderRepository.updateOrder(ord.id, {
        ...
        orderNote,
        grandTotalMinor:
          shippingfee + ord.subtotalMinor + taxTotalMinor - discountTotalMinor
      })
    ...
    if (paymentMethod === PaymentMethod.CREDIT_CARD) {
      ...
      await orderRepository.updateOrdersByCheckout(checkoutId.id, {
      ...
      await enqueueAutoCancel(
        {
          ...
          createdAt: (orderId[0] as any).createdAt.toString()
        },
        { delayMs: 15 * 60 * 1000 }
      )
  ...
  async createCart(
    ...
    for (let i = 0; i < productItemIds.length; i++) {
      const product = await orderRepository.findProductItemById(
        productItemIds[i]
      )
      let cardId: number
      ...
      if (product) {
        const findProduct = await orderRepository.findCartItem(
          productItemIds[i]
        )
        if (findProduct?.productItemId) {
          await orderRepository.updateCartItem(productItemIds[i], {
            quantity: quantities[i] + findProduct.quantity
          })
        } else {
  ...
  async customerCancel(checkoutId: string) {
    const findOrder = await orderRepository.findOrdersByCheckout(
      Number(checkoutId)
    )
    await orderRepository.updateOrdersByCheckout(Number(checkoutId), {
      status: OrderStatus.CUSTOMER_CANCELED
    })
    await orderRepository.updatePayment(Number(checkoutId), {
  ...
  async customerCancelV2(orderId: string) {
    ...
    await orderRepository.updateOrder(Number(orderId), {
      status: OrderStatus.CUSTOMER_CANCELED
    })
    if (findOrder) {
      await orderRepository.updatePayment(findOrder.checkoutId, {
        status: PaymentStatus.FAILED,
        pgwStatus: 'CANCELED'
      })
    }
  ...
  async revokeCancelOrder(refundId: string) {
    ...
    await orderRepository.updateRefundOrder(Number(refundId), {
    ...
    await orderRepository.updateOrder(refund.orderId, {
  ...
  async cancelOrder(
    orderId: number,
    body: { reason?: string; description?: string; contactEmail?: string },
    ...
    if (ord.status === OrderStatus.PENDING) {
      await sequelize.transaction(async (t) => {
        ...
        await ord.update({ status: OrderStatus.CUSTOMER_CANCELED } as any, {
          transaction: t
        })
      })
    ...
    const isDelivered =
      ...
      ord.status === OrderStatus.CANCELED_REFUND

    if (isDelivered) {
      if (!deliveredAt) throw new AppError('Missing deliveredAt timestamp', 422)
      if (
        differenceInDays(new Date(), new Date(deliveredAt)) >
        OrderPolicy.refundFromDeliveredDays
      ) {
        throw new AppError(
          'Refund request window after delivery has passed',
          422
        )
      }
      const since = addDays(
        new Date(deliveredAt),
        -OrderPolicy.refundRetryWindowDays
      )
      ...
      await sequelize.transaction(async (t) => {
        const refund = await orderRepository.createRefundOrder(
          {
            ...
            amountMinor: ord.grandTotalMinor,
        ...
        await ord.update({ status: OrderStatus.REFUND_REQUEST } as any, {
          transaction: t
        })
      })
      return {
        data: { id: ord.id, status: OrderStatus.REFUND_REQUEST, queued: false }
      }
    }
    ...
    if (ord.status === OrderStatus.PAID) {
      ...
      const hasDelivered = await orderRepository.findOrderStatusHistoryByStatus(
        ord.id,
      ...
      if (!hasDelivered) {
        ...
        return {
          data: { id: ord.id, status: OrderStatus.REFUND_REQUEST, queued: true }
        }
      }
      return {
        data: { id: ord.id, status: OrderStatus.REFUND_REQUEST, queued: false }
