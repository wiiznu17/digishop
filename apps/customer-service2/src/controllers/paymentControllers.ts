...
export const getNotify: RequestHandler = asyncHandler(
...
export const getCallBack: RequestHandler = asyncHandler(
  async (req: Request, res: Response) => {
    const reference = req.query.reference as string
    const process_status = req.query.process_status as string

    const { url } = await paymentService.getCallbackRedirectUrl(
      reference,
      process_status
    )
