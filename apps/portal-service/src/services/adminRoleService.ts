...
export class AdminRoleService {
  async listRoles(params: Record<string, string | undefined>) {
    ...
    const { rows, count } = await adminRoleRepository.findAndCountRoles(
      where, orderBy, offset, pageSize, attributes, group
    );
  ...
  async createRole(payload: { slug?: string; name?: string; description?: string }) {
    ...
    const created = await adminRoleRepository.createRole({ slug, name, description: description ?? null, isSystem: false });
    return { id: created.get("id") };
  }

  async updateRoleMeta(id: number, payload: { name?: string; description?: string }) {
    if (!Number.isFinite(id)) throw new BadRequestError("Invalid id");
    const { name, description } = payload;

    const role: any = await adminRoleRepository.findSimpleRoleById(id);
    if (!role) throw new NotFoundError("Not found");
    if (role.get("isSystem")) throw new BadRequestError("System role is not editable");
  ...
  async replaceRolePermissions(id: number, payload: { permissionIds?: number[]; permissionSlugs?: string[] }) {
    if (!Number.isFinite(id)) throw new BadRequestError("Invalid id");
    ...
    if (Array.isArray(payload.permissionIds) && payload.permissionIds.length > 0) {
      perms = await adminRoleRepository.findPermissionsByIds(payload.permissionIds);
    } else if (Array.isArray(payload.permissionSlugs) && payload.permissionSlugs.length > 0) {
      perms = await adminRoleRepository.findPermissionsBySlugs(payload.permissionSlugs);
    }
