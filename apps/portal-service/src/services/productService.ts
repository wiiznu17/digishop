...
export class ProductService {
  async listProducts(params: Record<string, string | undefined>) {
    ...
    if (categoryUuid) {
      const ids =
        await productRepository.collectCategoryDescendantIds(categoryUuid)
      if (ids.length === 0) {
        return {
          data: [],
    ...
    const priceOrder: SeqOrderItem = [
      col('minPriceMinor') as unknown as string,
      orderDir
    ]
  ...
  async moderateProduct(
    ...
    try {
      ...
      p.set({
        ...
        rejectReason:
          reqStatusEnum === ProductReqStatus.REJECT ? trimmedReason : null
      })
  ...
  async bulkModerateProducts(
    ...
    try {
      ...
      const [affected] = await productRepository.bulkModerateProducts(
        productUuids,
        payload,
        t
      )
