...
import { AppError, BadRequestError, ForbiddenError, NotFoundError } from "../errors/AppError";
...
export class AdminCredentialService {
  async sendInviteById(id: number, inviterId: number | undefined) {
    ...
    const recentUnaccepted = await adminCredentialRepository.findRecentUnacceptedInvite(email, cooldownSince);
    ...
    if (userRoles.length > 0) {
      const role = await adminCredentialRepository.findRoleById(userRoles[0].get("roleId") as number);
  ...
  async acceptInvite(payload: { token?: string; name?: string; password?: string }) {
    ...
    const invite: any = await adminCredentialRepository.findInviteByTokenHash(tokenHash);
    ...
    if (expiresAt.getTime() < now.getTime()) throw new BadRequestError("TOKEN_EXPIRED");

    const email = String(invite.get("email"));
    const passwordHash = await bcrypt.hash(password, ROUNDS);

    let admin: any = await adminCredentialRepository.findAdminByEmailIncludeDeleted(email);
    if (!admin) {
    ...
    if (roleSlugDefault) {
      const role = await adminCredentialRepository.findRoleBySlug(roleSlugDefault);
      if (role) {
        await adminCredentialRepository.findOrCreateUserRole(admin.get("id"), role.get("id") as number);
      }
    }
  ...
  async performReset(payload: { token?: string; password?: string }) {
    ...
    const tokenHash = sha256b64(token);
    const rec: any = await adminCredentialRepository.findResetByTokenHash(tokenHash);
