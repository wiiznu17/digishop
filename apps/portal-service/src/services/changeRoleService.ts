...
export class ChangeRoleService {
  ...
  async updateAdminRoles(id: number, payload: any, requesterRoleSlugs: string[]) {
    ...
    const roleSlugs = Array.isArray(payload?.roleSlugs) ? (payload.roleSlugs as string[]) : [];
    if (roleSlugs.length === 0) throw new BadRequestError("roleSlugs must not be empty");
    ...
    const wantRoleIds = new Set<number>(roles.map((r: any) => r.get("id") as number));
    const now = new Date();

    await sequelize.transaction(async (t: Transaction) => {
      const allRows = await changeRoleRepository.findAllAdminUserRolesActive(id, t);
