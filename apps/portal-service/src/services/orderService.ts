...
export class OrderService {
  ...
  async suggestCustomerEmails(q: string) {
    ...
    const perEmailAgg = await orderRepository.aggregateOrderEmailsForUsers(userIds);
    const perUserTotals = await orderRepository.aggregateUserOrderTotals(userIds);
  ...
  async listOrders(params: Record<string, string | undefined>) {
    const {
      q, status, dateFrom, dateTo, sortBy = "createdAt", sortDir = "desc", customerEmail, storeName,
    } = params;
    ...
    const { rows, count } = await orderRepository.findAndCountOrders(
      whereOrder, include, orderBy, offset, pageSize, group
    );
    ...
    const payload = {
      ...
      shipping: shippingInfo
        ? {
            ...
            deliveredAt: shippingInfo.get("deliveredAt") as Date | string | null,
            returnedToSenderAt: shippingInfo.get("returnedToSenderAt") as Date | string | null,
