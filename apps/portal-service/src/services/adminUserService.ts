...
export class AdminUserService {
  async listAdmins(params: Record<string, string | undefined>) {
    ...
    if (sortBy === 'name') orderBy.push([col('AdminUser.name'), dir])
      ...
      orderBy.push([col('AdminUser.last_login_at'), dir])
    ...
    const data = rows.map((u: any) => {
      ...
      const primaryRole = roleSlugs.includes('SUPER_ADMIN')
        ? 'SUPER_ADMIN'
        : (roleSlugs[0] ?? 'VIEWER')
      return {
        id: u.get('id'),
  ...
  async getAdminDetail(id: number) {
    ...
    const roleHistoryRows = await adminUserRepository.findRoleHistory(
      u.get('id') as number
    )
    ...
    if (canReinviteBase) {
      ...
      const recentInvite: any =
        await adminUserRepository.findRecentInvite(email)
      ...
      if (recentInvite) {
        ...
        const available = new Date(
          createdAt.getTime() + REINVITE_COOLDOWN_MIN * 60 * 1000
        )
        ...
        if (now < available.getTime()) {
          canReinvite = false
          reinviteCooldownSeconds = Math.ceil(
            (available.getTime() - now) / 1000
          )
        }
      }
    }
  ...
  async createAdmin(payload: {
    ...
    await adminUserRepository.createUserRole(
      created.get('id') as number,
