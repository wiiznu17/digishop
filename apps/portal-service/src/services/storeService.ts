...
export class StoreService {
  async listStores(params: Record<string, string | undefined>) {
    ...
    if (from && to)
      (storeWhere as any)['createdAt'] = { [Op.between]: [from, to] }
    ...
    const orderTotalExpr = fn(
      'COALESCE',
      fn('SUM', col('storeOrders.grand_total_minor')),
      0
    )
    ...
    if (salesMinMinor !== null)
      havingAnd.push(sqlWhere(orderTotalExpr, { [Op.gte]: salesMinMinor }))
    if (salesMaxMinor !== null)
      havingAnd.push(sqlWhere(orderTotalExpr, { [Op.lte]: salesMaxMinor }))

    const orderCountMinInt = asInt(orderCountMin, -1)
    const orderCountMaxInt = asInt(orderCountMax, -1)
    if (orderCountMin && orderCountMinInt >= 0)
      havingAnd.push(sqlWhere(orderCountExpr, { [Op.gte]: orderCountMinInt }))
    if (orderCountMax && orderCountMaxInt >= 0)
      havingAnd.push(sqlWhere(orderCountExpr, { [Op.lte]: orderCountMaxInt }))

    const dir = String(sortDir).toUpperCase() === 'ASC' ? 'ASC' : 'DESC'
    const orderBy: any[] = []
    if (sortBy === 'status') orderBy.push([col('Store.status'), dir])
    else if (sortBy === 'storeName')
      orderBy.push([col('Store.store_name'), dir])
    ...
    const storeIdsOnPage = rows
      .map((s: any) => Number(s.get('id')))
      .filter(Boolean)
    const productCountsRaw =
      await storeRepository.getProductCountsForStores(storeIdsOnPage)
    ...
    const data = rows.map((s: any) => ({
      ...
      ownerName: [s.owner?.get('firstName'), s.owner?.get('lastName')]
        .filter(Boolean)
        .join(' '),
  ...
  async getStoreDetail(id: number) {
    ...
    const monthlyMap = new Map<
      string,
      { totalSalesMinor: number; orderCount: number }
    >()
    ...
    const recentItems =
      await storeRepository.getRecentOrderItems(recentOrderIds)

    const itemsByOrder = new Map<
      number,
      Array<{ productId: number; productName: string; uuid: string }>
    >()
