...
export class UserService {
  async listUsers(params: Record<string, string | undefined>) {
    const { q = "", dateFrom, dateTo, sortBy = "createdAt", sortDir = "desc" } = params;
    ...
    const { rows, count } = await userRepository.findAndCountUsers(
      whereClause, having, orderBy, offset, pageSize, attributes, group
    );
  ...
  async getUserDetail(id: number) {
    ...
    const summary = summaryRows[0] || { totalOrders: 0, totalSpentMinor: 0, averageOrderMinor: 0, lastOrderAt: null };

    const latestOrders = await userRepository.findLatestOrders(id);

    const monthExpression = fn("DATE_FORMAT", col("Order.created_at"), "%Y-%m");
    const monthlyAggregateRows = await userRepository.findMonthlyOrderAggregate(id, monthExpression);
