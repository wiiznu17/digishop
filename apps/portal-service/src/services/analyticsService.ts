...
const toNumber = (v: unknown) =>
  Number.isFinite(Number(v ?? 0)) ? Number(v) : 0
...
export class AnalyticsService {
  async getKpis(query: Record<string, string | undefined>) {
    ...
    const refunds: any = await analyticsRepository.getKpisRefunds(
      buildRefundWhere(query)
    )
    const refundCount = toNumber(refunds?.[0]?.refunds)

    const repeatRows: any = await analyticsRepository.getKpisRepeatRows(where)
    const usersWith2Plus = repeatRows.filter(
      (r: any) => toNumber(r.c) >= 2
    ).length
  ...
  async getStoreLeaderboard(query: Record<string, string | undefined>) {
    ...
    const segment =
      (query.segment as 'ALL' | 'TOP' | 'LOW' | undefined) ?? 'ALL'
    ...
    const allRows: any = await analyticsRepository.getStoreLeaderboard(
      baseWhere,
      nameFilter
    )
    ...
    if (segment === 'TOP') {
      ...
      sorted = [...materialized]
        .sort((a, b) => a.gmvMinor - b.gmvMinor)
        .slice(0, 10)
    } else {
