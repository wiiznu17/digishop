...
export class CategoryService {
  ...
  async listCategories(params: { parentUuid?: string; q?: string; page?: string; pageSize?: string; mode?: "flat" }) {
    ...
    if (mode === "flat") {
      ...
      const rows: any = await categoryRepository.findAllCategoriesParams(
        where, [["name", "ASC"]], ["uuid", "name", "parentId", "createdAt", "updatedAt"]
      );

      const parentIds = rows.map((r: any) => r.parentId).filter(Boolean) as number[];
      const parents = parentIds.length > 0 ? await categoryRepository.findAllCategoriesByIds(parentIds) : [];
    ...
    const { count, rows } = await categoryRepository.findAndCountCategories(
      where, [["name", "ASC"]], ps, (p - 1) * ps, ["id", "uuid", "name", "parentId", "createdAt", "updatedAt"]
    );
  ...
  async updateCategory(uuid: string, payload: { name?: string; parentUuid?: string | null }) {
    const t = await sequelize.transaction();
    try {
      ...
      if (parentUuid !== undefined) {
        if (parentUuid === null) parentId = null;
          ...
          const p: any = await categoryRepository.findCategoryByUuid(parentUuid, t);
          if (!p) throw new BadRequestError("Parent not found");
  ...
  async deleteCategory(uuid: string) {
    ...
    try {
      ...
      if (e instanceof ConflictError && e.message.startsWith("CATEGORY_HAS_PRODUCTS")) {
  ...
  async moveProducts(uuid: string, targetCategoryUuid: string) {
    ...
    try {
      ...
      const dst: any = await categoryRepository.findCategoryByUuid(targetCategoryUuid, t);

      if (!src || !dst) throw new BadRequestError("Source or target not found");
      if (src.uuid === dst.uuid) throw new BadRequestError("Target must be different from source");
      ...
      const [affected] = await categoryRepository.moveProducts(descIds, dst.id, t);
