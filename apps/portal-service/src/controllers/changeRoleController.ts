...
export const adminUpdateAdminRoles: RequestHandler = asyncHandler(async (req: Request, res: Response) => {
  const id = Number(req.params.id);
  const requesterRoleSlugs = (req as any)?.roleSlugs || [];
  const result = await changeRoleService.updateAdminRoles(id, req.body, requesterRoleSlugs);
  res.json(result);
});
