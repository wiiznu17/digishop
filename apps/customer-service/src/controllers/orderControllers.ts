...
export const createOrderId = async (
  ...
  if (Object.keys(groupStoreId).length > 1)
    return res.json({ error: 'order more than one store' })
  try {
    if (user) {
...
export const cancelOrder = async (req: Request, res: Response) => {
  ...
  if (transitStatuses.includes(order.status as OrderStatus)) {
    return res.status(422).json({
      error: 'Cancellation/refund not allowed in this status.'
    })
  }
  ...
  if (order.status === OrderStatus.PENDING) {
    await sequelize.transaction(async (t) => {
      ...
      await order.update({ status: OrderStatus.CUSTOMER_CANCELED } as any, {
        transaction: t
      })
    })
    return res.json({
      data: { id: order.id, status: OrderStatus.CUSTOMER_CANCELED }
    })
  }
  ...
  if (isDelivered) {
    ...
    if (
      ...
      return res
        .status(422)
        .json({ error: 'Refund request window after delivery has passed' })
    }
    ...
    return res.json({
      data: { id: order.id, status: OrderStatus.REFUND_REQUEST, queued: false }
    })
  }
  ...
  if (order.status === OrderStatus.PAID) {
    ...
    if (!hasDelivered) {
      ...
      return res.json({
        data: { id: order.id, status: OrderStatus.REFUND_REQUEST, queued: true }
      })
    }
    return res.json({
      data: { id: order.id, status: OrderStatus.REFUND_REQUEST, queued: false }
  ...
  return res
    .status(422)
    .json({ error: `Cancel/Refund not allowed in status ${order.status}` })
}
