...
export default function StoreScreen() {
  ...
  if (!store) return <View className="flex-1 items-center justify-center"><Text className="text-gray-500">Store not found</Text></View>;
  ...
  return (
    <View className="flex-1 bg-gray-50">
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View className="items-center mt-12"><Text className="text-5xl mb-3">📭</Text><Text className="text-gray-500">No products in this store</Text></View>
        }
        renderItem={({ item }: { item: Product }) => {
          const price = item.items?.[0]?.priceMinor;
          const img = item.images?.[0]?.url || item.items?.[0]?.productItemImage?.[0]?.url;
          return (
            <TouchableOpacity
              className="flex-1 bg-white rounded-2xl overflow-hidden shadow-sm mb-3"
              onPress={() => router.push({ pathname: "/product/[id]", params: { id: item.uuid } })}
            >
              {img ? (
                <Image source={{ uri: img }} className="w-full h-36" resizeMode="cover" />
              ) : (
                <View className="w-full h-36 bg-gray-100 items-center justify-center"><Text className="text-4xl">📦</Text></View>
              )}
              <View className="p-3">
                <Text className="text-gray-900 font-semibold text-sm" numberOfLines={2}>{item.name}</Text>
                {price !== undefined && <Text className="text-blue-700 font-bold text-sm mt-1">{fmt(price)}</Text>}
              </View>
            </TouchableOpacity>
          );
        }}
