...
import { fetchUserCart, deleteCart, createOrderId } from "../../src/utils/requestOrderUtils";
...
interface CartItem {
  ...
  productItem?: { sku: string; priceMinor: number; productItemImage?: {url: string}[]; product?: { id: number; name: string; uuid: string; store?: { id: number } } };
}
...
export default function CartScreen() {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const res = await createOrderId({ customerId: user.id, orderData }) as any;
      router.push({ pathname: "/checkout", params: { orderCode: res?.data } } as any);
    } catch (e: any) {
      Alert.alert("Checkout Error", e?.response?.data?.error || "Failed to create order");
    }
  };
        ...
        <>
              ...
              <View className="bg-white rounded-2xl p-4 mb-3 shadow-sm flex-row gap-3">
                <View className="w-16 h-16 bg-gray-100 rounded-xl items-center justify-center">
                  {item.productItem?.productItemImage?.[0]?.url ? (
                    <Image source={{ uri: item.productItem.productItemImage[0].url }} className="w-16 h-16 rounded-xl" resizeMode="cover" />
                  ) : (
                    <Text className="text-2xl">📦</Text>
                  )}
                </View>
                <View className="flex-1">
                  <Text className="text-gray-900 font-semibold" numberOfLines={1}>{item.productItem?.product?.name || item.productItem?.sku}</Text>
                  <Text className="text-gray-500 text-sm mt-1">Qty: {item.quantity}</Text>
                  <Text className="text-blue-700 font-bold mt-1">{fmt(item.unitPriceMinor * item.quantity)}</Text>
                </View>
          ...
          <View className="bg-white px-6 py-4 border-t border-gray-100 shadow-lg">
            <View className="flex-row justify-between mb-4">
              <Text className="text-gray-600 text-base">Total ({items.length} items)</Text>
              <Text className="text-blue-700 text-xl font-bold">{fmt(total)}</Text>
            </View>
            <TouchableOpacity className="bg-blue-700 rounded-xl py-4 items-center" onPress={handleCheckout}>
