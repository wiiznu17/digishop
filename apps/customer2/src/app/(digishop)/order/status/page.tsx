...
export default function OrderStatus() {
  ...
  const [isShowCancel, setIsShowCancel] = useState<CancelRefundProps>({
    shown: false,
    id: undefined
  })
  const [isShowRefund, setIsShowRefund] = useState<CancelRefundProps>({
    shown: false,
    id: undefined
  })
  ...
  const [selectStatus, setSelectStatus] =
    useState<keyof typeof StateConfig>('PENDING')
  useEffect(() => {
    const fetchData = async () => {
      if (!user) return
      const data = (await fetchUserOrders(user.id)) as {
        body: OrderDetail[]
        count: number
      }
      const { body } = data
          ...
          {Object.entries(StateConfig).map(([state, config]) => {
            ...
            return (
              <button
                key={state}
                onClick={() =>
                  handleChangeState(state as keyof typeof StateConfig)
                }
                className={`
            ...
            filterOrder.map((order, index: number) => (
              <div key={index}>
                <OrderCard
                  ...
                  setSelectShowCancel={setSelectShowCancel}
                  selectShowCancel={selectShowCancel}
