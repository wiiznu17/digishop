...
export function AddressCardForSetting({ item }: AddressCardForSetting) {
  ...
  return (
    <div
      className={`relative border  text-lg rounded-2xl mb-3 p-4 ${rubik.className}`}
    >
      <div className="">
        <div className="flex">
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-gray-800 text-lg">{formatAddress(item)}</p>
            </div>
          </div>
          <div className="flex items-center space-x-4">
            <div className="w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center">
      ...
      <button
        className="absolute right-15 top-5 hover:bg-gray-300/50 cursor-pointer p-2 rounded-full"
        onClick={() => setEditAddShow(true)}
      >
        <Pen />
      </button>
      {!item.isDefault && (
        <button
          className="absolute right-3 top-5 hover:bg-red-300/50 cursor-pointer p-2 rounded-full"
      ...
      <EditAddress
        item={item}
        setEditAddShow={setEditAddShow}
        editAddShow={editAddShow}
      />
    </div>
  )
}
...
export function AddressCardForOrder({
  item,
  select
}: AddressCardForOrderProps) {
  const formatAddress = (items: Address): string => {
    return [
      items.address_number,
      items.building,
  ...
  return (
    <div
      className={`relative border text-lg rounded-2xl mb-3 p-4 ${item.id !== select?.id ? ' border-gray-400' : ''}`}
    >
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex-1">
              ...
              <p className=" text-lg">{formatAddress(item)}</p>
            </div>
          </div>
          <div className="flex items-center space-x-4">
            <div className="w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center">
